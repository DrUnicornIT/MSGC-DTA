...
def load_data(dataset):
    affinity = pickle.load(open('/kaggle/input/msgc-dta/MSGC-DTA/' + dataset + '/affinities', 'rb'), encoding='latin1')
    if dataset == 'davis':
...
def process_data(affinity_mat, dataset, num_pos, pos_threshold):
    dataset_path = '/kaggle/input/msgc-dta/MSGC-DTA/' + dataset + '/'
...
def get_target_molecule_graph(proteins, dataset):
    msa_path = '/kaggle/input/msgc-dta/MSGC-DTA/' + dataset + '/aln'
    contac_path = '/kaggle/input/msgc-dta/MSGC-DTA/' + dataset + '/pconsc4'    
