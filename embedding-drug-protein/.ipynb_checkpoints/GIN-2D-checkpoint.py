# Source https://github.com/wenx00/dgl-lifesci/blob/master/examples/molecule_embeddings/main.py
# -*- coding: utf-8 -*-
#
...
def main(args, dataset, name_dataset):
    ...
    print(f"Shape: {mol_emb.shape}")
    np.save(args['out_dir'] + f'/unique_drug_GIN_EMB_{name_dataset.lower()}.npy', mol_emb)
...
if __name__ == '__main__':
    ...
    dataset, success = graph_construction_and_featurization(drug_dict.values())
    np.save(args['out_dir'] + '/mol_parsed.npy', np.array(success))
    main(args, dataset, args['dataset'])